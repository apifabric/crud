...
resourcename="crud"  # lower case, only
resourcegroup="crud_rg"
...
# git clone https://github.com/apilogicserver/crud.git
# cd crud
...
az group create --name crud_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group crud_rg --sku S1 --is-linux
az container create --resource-group crud_rg --name crud --image apilogicserver/crud:latest --dns-name-label crud --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//crud.westus.azurecontainer.io:5656'
